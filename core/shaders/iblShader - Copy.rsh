...
vec3 CalcIBL(vec3 N, vec3 V, vec3 F0, vec3 albedo_color, float metallic_color, float roughness_color)
{
	...
	// Correct the reflection with Parallax Correction tech
	if(use_parallax_correction)
	{
		...
		vec3 FirstPlaneIntersect = (BoxMax - FragPos) / ReflDirectionWS;
		vec3 SecondPlaneIntersect = (BoxMin - FragPos) / ReflDirectionWS;
		...
		//vec3 BoxDiff = (BoxMax - BoxMin);
		//float minDim = min(BoxDiff.z, min(BoxDiff.x, BoxDiff.y));
		//vec3 BoxScale = vec3(minDim) / BoxDiff;
...
vec3 CalcPointLight(PointLight light, vec3 V, vec3 N, vec3 F0, vec3 albedo_color, float metallic_color, float roughness_color)
{
	...
	{
		shadow = PointShadowCalculation(FragPos, light.position, light.raduis, light.Bias, light.shadow_index);
