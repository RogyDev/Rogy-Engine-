...
SpotLight* Renderer::CreateSpotLight(EnttID ent_id)
{
	...
	materials.PbrShader.setBool(("sp_lights[" + std::to_string(m_SpotLights.size() - 1) + "].active").c_str(), true);
	return pl;
}
// ------------------------------------------------------------------------
bool Renderer::RemoveSpotLight(EnttID ent_id)
{
	for (size_t i = 0; i < m_SpotLights.size(); i++)
	{
		if (m_SpotLights[i]->light_id == ent_id)
		{
			...
			delete sl;
			materials.PbrShader.use();
			materials.PbrShader.setBool(("sp_lights[" + std::to_string(m_SpotLights.size()) + "].active").c_str(), false);
			return true;
		}
	}
	return false;
...
PointLight* Renderer::CreatePointLight(EnttID ent_id)
{
	...
	materials.PbrShader.use();
	materials.PbrShader.setBool(("p_lights[" + std::to_string(m_PointLights.size() - 1) + "].active").c_str(), true);
	return pl;
}
// ------------------------------------------------------------------------
bool Renderer::RemovePointLight(EnttID ent_id)
{
	for (size_t i = 0; i < m_PointLights.size(); i++)
	{
		if (m_PointLights[i]->light_id == ent_id)
		{
			...
			delete pl;
			materials.PbrShader.use();
			materials.PbrShader.setBool(("p_lights[" + std::to_string(m_PointLights.size()) + "].active").c_str(), false);
			return true;
		}
	}
	return false;
...
DirectionalLight* Renderer::CreateDirectionalLight()
{
	if (m_DirectionalLight == nullptr)
	{
		...
		materials.PbrShader.use();
		materials.PbrShader.SetFloat("dirLight.active", true);
...
bool Renderer::RemoveDirectionalLight()
{
	if (m_DirectionalLight != nullptr)
	{
		...
		materials.PbrShader.use();
		materials.PbrShader.SetFloat("dirLight.active", false);
		return true;
	}
	return false;
}
...
void Renderer::RenderScene(Camera& cam, bool static_only, GLuint target_frambuffer, int resolu)
{
	...
	if (m_RenderBuffer.ForwardRenderCommands.size() > 0)
	{
		...
		if (m_DirectionalLight != nullptr && m_DirectionalLight->Active && m_DirectionalLight->CastShadows)
		{
			...
			for (size_t i = 0; i < m_ShadowMapper.cascades.size(); i++)
			{
				...
				for (size_t i = 0; i < m_RenderBuffer.ForwardRenderCommands.size(); i++)
				{
					...
					if (!m_RenderBuffer.ForwardRenderCommands[i].cast_shadows)
						continue;
		...
		if (!m_PointLights.empty())
		{
			for (size_t i = 0; i < m_PointLights.size(); i++)
			{
				if (m_PointLights[i]->CastShadows == true && m_PointLights[i]->shadow_index != -1)
				{
					m_PointLights[i]->visible = frustum.sphereInFrustum(m_PointLights[i]->Position, m_PointLights[i]->Raduis);
					...
					{
						if (static_only && !m_RenderBuffer.ForwardRenderCommands[j].is_static)
							continue;

						if (!m_RenderBuffer.ForwardRenderCommands[j].cast_shadows)
							continue;
		...
		if (!m_SpotLights.empty())
		{
			for (size_t i = 0; i < m_SpotLights.size(); i++)
			{
				...
				{
					m_SpotLights[i]->visible = frustum.sphereInFrustum(m_SpotLights[i]->Position, m_SpotLights[i]->Raduis);
					...
					for (size_t j = 0; j < m_RenderBuffer.ForwardRenderCommands.size(); j++)
					{
						...
						if (!fr.IsBoxVisible(m_RenderBuffer.ForwardRenderCommands[j].bbox.BoxMin, m_RenderBuffer.ForwardRenderCommands[j].bbox.BoxMax))
							continue;
		...
		/*if (postProc.Initialized && postProc.Use)
		{
			materials.DebugSH.use();
			glActiveTexture(GL_TEXTURE0);
		...
		//useShader(materials.PbrShader, cam);
		...
		// Set Fog config
		// -----------------------------------------------------------------
		pbrShader->setBool("Fog.active", usefog);
		if (usefog)
		{
		...
		for (size_t i = 0; i < m_RenderBuffer.ForwardRenderCommands.size(); i++)
		{
			...
			if (lastMaterial != m_RenderBuffer.ForwardRenderCommands[i].Material->name)
			{
				...
				if (m_RenderBuffer.ForwardRenderCommands[i].Material->tex_albedo != nullptr)
				{
					...
					pbrShader->setBool("use_tex_albedo", false);
				...
				if (m_RenderBuffer.ForwardRenderCommands[i].Material->tex_metal != nullptr)
				{
					...
					m_RenderBuffer.ForwardRenderCommands[i].Material->tex_metal->useTexture();
				}
				else
					pbrShader->setBool("use_tex_metal", false);
				// roughness map 
				// -------------------------------------------------------
				if (m_RenderBuffer.ForwardRenderCommands[i].Material->tex_rough != nullptr)
				{
					...
					m_RenderBuffer.ForwardRenderCommands[i].Material->tex_rough->useTexture();
				}
				else
					pbrShader->setBool("use_tex_rough", false);
				...
				if (m_RenderBuffer.ForwardRenderCommands[i].Material->tex_normal != nullptr)
				{
					...
					m_RenderBuffer.ForwardRenderCommands[i].Material->tex_normal->useTexture();
				}
				else
					pbrShader->setBool("use_tex_normal", false);
				...
				if (m_RenderBuffer.ForwardRenderCommands[i].Material->use_emission)
				{
					...
					if (m_RenderBuffer.ForwardRenderCommands[i].Material->tex_emission != nullptr)
					{
						...
						m_RenderBuffer.ForwardRenderCommands[i].Material->tex_emission->useTexture();
					}
					else
						pbrShader->setBool("use_tex_emission", false);
				}
			...
			// Check ReflectionProp intersection
			...
			if (!cam.temp_cam)
			{
				for (size_t j = 0; j < m_ReflectionProbes.size(); j++)
				{
					if (m_ReflectionProbes[j]->GetBBox().Intersect(m_RenderBuffer.ForwardRenderCommands[i].bbox))
					{
						...
						pbrShader->setBool("use_parallax_correction", m_ReflectionProbes[j]->BoxProjection);
						pbrShader->SetVec3("mRefPos", m_ReflectionProbes[j]->Position);
						pbrShader->SetVec3("mBoxMin", m_ReflectionProbes[j]->GetBBox().BoxMin);
						pbrShader->SetVec3("mBoxMax", m_ReflectionProbes[j]->GetBBox().BoxMax);
				...
				{
					pbrShader->setBool("use_parallax_correction", false);
					m_cache.BindTexture(TEX_IRRADIANCE_MAP, m_SkyCapture.Irradiance);
					m_cache.BindTexture(TEX_PREFILTER_MAP, m_SkyCapture.Prefiltered);
				}
			}
			else pbrShader->setBool("use_parallax_correction", false);

			// Lightmap if availibale 
			// -------------------------------------------------------
	...
	//glActiveTexture(GL_TEXTURE0);
	//glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap); //m_SkyCapture.Irradiance
	glDisable(GL_CULL_FACE);
	//renderSphere();
	renderCube();
	glEnable(GL_CULL_FACE);
...
void Renderer::EndFrame()
{
	...
	if (postProc.Use)
	{
		...
		{
			postProc.fps = fps;
			postProc.cam_velocity = -glm::vec2(MainCam.rot_vector.y, -MainCam.rot_vector.x);
		}
		glViewport(left_scr_pos, 0, SCR_weight, SCR_height);
		renderQuad();
	}
...
bool Renderer::SaveMaterial(Material* mat, const char* mPath)
{
	if (mat == nullptr)
		return false;
	//cout << "saving existed mat : " << mPath << endl;

	std::ofstream os(mPath, std::ios::binary);

	cereal::BinaryOutputArchive archive(os);

	mat->serialize<cereal::BinaryOutputArchive>(archive);
...
bool Renderer::CreateMaterialOnDisk(const char* mPath)
{
	std::ofstream os(mPath, std::ios::binary);
	cereal::BinaryOutputArchive archive(os);
	materials.GetMaterialN("")->serialize<cereal::BinaryOutputArchive>(archive);
	os.close();
	return true;
}
// ------------------------------------------------------------------------
...
void Renderer::BakeLightMaps(int meshIndex)
{
	...
	std::cout << "\n Building Mesh = " << ii + 1 << "//" << lm_Count << std::endl;
	...
	lmSetGeometry(ctx, dArray,
		...
		LM_FLOAT, (unsigned char*)&m_RenderBuffer.ForwardRenderCommands[ii].Mesh->vertices[0] + offsetof(Vertex, TexCoords), sizeof(Vertex),
