...
public:
	...
	bool Frustum::sphereInFrustum(glm::vec3 &minp, float radius);
...
inline bool Frustum::pointInFrustum(glm::vec3 &minp) {
...
inline bool Frustum::sphereInFrustum(glm::vec3 &minp, float radius) {

	float distance;
	bool result = true;

	for (int i = 0; i < 6; i++) {
		distance = glm::dot(m_planes[i], glm::vec4(minp.x, minp.y, minp.z, 1.0f));
