...
{
    vec4 pos[MAX_LIGHT_COUNT];
    vec4 color[MAX_LIGHT_COUNT];
	bool cast_shadows[MAX_LIGHT_COUNT];
	int  shadow_index[MAX_LIGHT_COUNT];
	float Bias[MAX_LIGHT_COUNT];
} u_Light;
...
#if SHADOW_QUALITY == 2 
		// PCF
		vec2 texelSize = 1.5 / textureSize(shadowMaps, 0);
	
		for(int x = -1; x <= 1; ++x)
		{
			for(int y = -1; y <= 1; ++y)
			{		
				shadow += (1.0 - texture( shadowMaps, vec3(projCoords.xy + normalize(vec2(x, y)) * texelSize , projCoords.z - bias) ));
			}    
		}
		shadow /= 9.0;
