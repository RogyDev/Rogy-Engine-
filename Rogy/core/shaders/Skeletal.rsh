...
out vec4 spot_MVP_SPACE[MAX_LIGHT_COUNT_SK];

uniform mat4 projection;
uniform mat4 view;
uniform mat4 model;

uniform mat4 gLightWVP[MAX_LIGHT_COUNT_SK];
uniform mat4 spot_MVP[MAX_LIGHT_COUNT_SK];
...
void main()
{
	...
	for(int i = 0 ; i < MAX_BONE_INFLUENCE ; i++)
    {
		...
		vec4 worldNormal = finalBonesMatrices[boneIds[i]] * vec4(aNormal, 0.0);
	...
	for (int i = 0 ; i < NUM_CASCADES ; i++) 
	 ...
	 for(int i = 0 ; i < MAX_LIGHT_COUNT_SK ; i++)
...
// ligthing
uniform vec3 CamPos;
uniform PointLight[MAX_LIGHT_COUNT_SK] p_lights;
uniform SpotLight[MAX_LIGHT_COUNT_SK] sp_lights;
uniform DirLight dirLight;

// Point Shadows
uniform samplerCube tex_shadows[MAX_LIGHT_COUNT_SK];

// Spot Shadows
uniform sampler2DShadow texSpot_shadows[MAX_LIGHT_COUNT_SK];
in vec4 spot_MVP_SPACE[MAX_LIGHT_COUNT_SK];
