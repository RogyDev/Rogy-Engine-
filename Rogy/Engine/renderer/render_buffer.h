...
public:
	...
	std::vector<RenderCommand> ForwardRenderCommands;
	std::vector<RenderCommand> AlphaRenderCommands;

	// pushes render state relevant to a single render call to the command buffer.
	void Push(Mesh* mesh, Material* material, glm::mat4 transform, BBox box, bool cast_shadows = true, glm::vec3 pos = glm::vec3(0), bool is_static = false, std::string lmPath = "");
