...
void TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	...
	auto cindex = (size_t)GetCharacterIndex(mState.mCursorPosition);
	while (aAmount-- > 0)
	{
...
float TextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
{
	...
	float distance = 0.0f;
	float spaceSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ", nullptr, nullptr).x;
	size_t colIndex = (size_t)GetCharacterIndex(aFrom);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Lua()
{
	...
	if (!inited)
	{
		static const char* const keywords[] = { "self",
		...
		for (auto& k : identifiers)
		{
			...
			if (k == "class")
			{
				id.mDeclaration = "function defines a new class.";
			}
			else if (k == "print")
			{
				id.mDeclaration = "void print(...)\n\nReceives any number of arguments and prints their values to the console.";
			}
			//id.mDeclaration = "Built-in function";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
